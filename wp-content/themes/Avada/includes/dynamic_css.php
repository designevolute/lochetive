...
function avada_dynamic_css_array( $original_css = array() ) {
	...
	if ( Avada()->settings->get( 'responsive' ) ) {
		...
		*/
		...
		if ( class_exists( 'Tribe__Events__Main' ) ) {
			$css[ $six_fourty_media_query ]['.tribe-ticket .quantity']['max-width'] = '122px';
			$css[ $six_fourty_media_query ]['.tribe-ticket .quantity']['width'] = '100%';
		}
		...
		*/
		...
		if ( class_exists( 'Tribe__Events__Main' ) ) {
			...
			$css[ $four_eigthy_media_query ][ $dynamic_css_helpers->implode( $elements ) ]['padding-left']  = '7px';
			$css[ $four_eigthy_media_query ][ $dynamic_css_helpers->implode( $elements ) ]['padding-right'] = '7px';
		}
